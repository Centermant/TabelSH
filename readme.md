...
|   └── .env                 # Файл конфигурации (создаётся вручную, включен в .gitignore)
├── public/                  # Фронтенд-часть приложения
│   ├── css/                 # Стили
│   ├── js/                  # JavaScript-модули
│   └── assets/              # Статические ресурсы (библиотеки)
...
2.  Перейдите на вкладку **"SQL команда"**.

3.  **Вставьте исправленный SQL-скрипт** из файла [`bd.txt`](bd.txt), который находится в корне этого репозитория.

    **Важно!** В оригинальном экспорте Adminer есть ошибка в определении таблицы `users`. Убедитесь, что строка с полем `applications` выглядит **именно так**:
